...
function findLinkedInTab(): { windowIndex: number; tabIndex: number } | null {
  ...
  try {
    ...
    if (
      activeUrl.includes("linkedin.com/mynetwork/invite-connect/connections")
    ) {
...
(function() {
  ...
  executeJS(windowIndex, tabIndex, extractJs);
  ...
  executeJS(
    windowIndex,
    tabIndex,
    `localStorage.removeItem("__lcs_connections__")`,
  );
...
export async function scrapeViaAppleScript(opts: {
  ...
  if (platform() !== "darwin") {
    throw new Error(
      "AppleScript mode only works on macOS. Use --playwright mode on other platforms.",
    );
  }
  ...
  console.log(
    `Found LinkedIn connections tab (window ${tab.windowIndex}, tab ${tab.tabIndex})\n`,
  );
